// scripts/deploy-vehicle-manager.js (ESM VERSION)
...
async function main() {
  ...
  console.log("⚠️  You are about to deploy VehicleManager to 0G MAINNET!");
  ...
  console.log("\n🚗 Deploying VehicleManager contract...");
  
  // Get contract factory
  const VehicleManager = await hre.ethers.getContractFactory("VehicleManager");
  
  // Estimate deployment cost
  try {
    const deploymentData = VehicleManager.getDeployTransaction();
  ...
  const contract = await VehicleManager.deploy();
  ...
  console.log("─────────────────────────────────────────────────");
  console.log(`VEHICLE_CONTRACT_ADDRESS=${contractAddress}`);
  console.log("─────────────────────────────────────────────────");
  ...
  console.log("   ✓ Total Purchases:", stats[0].toString());
  console.log("   ✓ Total Switches:", stats[1].toString());
  ...
  console.log("   [ ] Test a vehicle switch with small amount");
