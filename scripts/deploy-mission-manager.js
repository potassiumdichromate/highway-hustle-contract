// scripts/deploy-mission-manager.js (ESM VERSION)
...
async function main() {
  ...
  console.log("⚠️  You are about to deploy MissionManager to 0G MAINNET!");
  ...
  console.log("\n🎯 Deploying MissionManager contract...");
  ...
  try {
    const deploymentData = MissionManager.getDeployTransaction();
  ...
  const contract = await MissionManager.deploy();
  ...
  console.log("─────────────────────────────────────────────────");
  console.log(`MISSION_CONTRACT_ADDRESS=${contractAddress}`);
  console.log("─────────────────────────────────────────────────");
  ...
  console.log("   ✓ Total Completions:", stats[1].toString());
  console.log("   ✓ Total Achievements:", stats[2].toString(), "(8 default achievements created)");
  console.log("   ✓ Total Players:", stats[3].toString());
  console.log("   ✓ Owner:", stats[4]);

  // List default achievements
  ...
  for (let i = 0; i < achievementIds.length; i++) {
    const achievement = await contract.getAchievement(achievementIds[i]);
    console.log(`   ${i + 1}. ${achievementIds[i]}: ${achievement[0]}`);
  }
  ...
  console.log("   [ ] Create initial missions via backend");
  console.log("   [ ] Test achievement unlock");
  console.log("   [ ] Test mission completion");
